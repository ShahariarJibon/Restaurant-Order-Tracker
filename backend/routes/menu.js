...
router.get('/public/:restaurantId', async (req, res) => {
  const restaurant = await queryOne('SELECT id, name, currency, logo, plan, status, payment_qr_bkash, payment_qr_nagad, payment_qr_rocket FROM restaurants WHERE id = ?', [req.params.restaurantId]);
